...
-- a. Excluir os registros de PEDIDO em que o número seja ímpar, a matrícula do vendedor 102 e o endereço de entrega não seja nulo.
-- Observação: Realizar a exclusão e verificar que os registros foram excluídos apenas na transação.

-- Excluir itens de produtos relacionados aos pedidos
...
WHERE NUMERO_PED IN (
    ...
    WHERE MOD(NUMERO, 2) = 1
      ...
      AND (RUA IS NOT NULL OR NR IS NOT NULL OR BAIRRO IS NOT NULL OR COMPLEMENTO IS NOT NULL OR CIDADE IS NOT NULL OR UF IS NOT NULL OR CEP IS NOT NULL)
);

-- Excluir registros de pedidos
DELETE FROM PEDIDO
WHERE MOD(NUMERO, 2) = 1
  AND MATRICULA_VEN = 102
  AND (RUA IS NOT NULL OR NR IS NOT NULL OR BAIRRO IS NOT NULL OR COMPLEMENTO IS NOT NULL OR CIDADE IS NOT NULL OR UF IS NOT NULL OR CEP IS NOT NULL);
...
-- b. Excluir os itens de produtos do menor número de pedido na última data de inclusão de pedido (a data mais recente).
-- Observação: Realizar a exclusão e verificar que os registros foram excluídos apenas na transação.

BEGIN
    -- Encontrar a data mais recente de inclusão de pedido
    DECLARE
        v_ultima_data DATE;
        v_menor_numero_ped NUMBER;
    BEGIN
        SELECT MAX(DATA) INTO v_ultima_data FROM PEDIDO;

        -- Encontrar o menor número de pedido na data mais recente
        SELECT MIN(NUMERO) INTO v_menor_numero_ped FROM PEDIDO WHERE DATA = v_ultima_data;

        -- Excluir itens de produtos do menor número de pedido na última data de inclusão de pedido
        DELETE FROM ITEM_PRODUTO
        WHERE NUMERO_PED = v_menor_numero_ped;
...
-- b. Listar distintamente os clientes PJ. Mostrar: Nome do cliente (CLIENTE), CNPJ e CIDADE onde reside.
-- Ordenados pelo nome do cliente ascendente e, dentro deste, pela cidade descendente.
-- RESOLUÇÃO:
SELECT DISTINCT C.NOME AS CLIENTE, PJ.CNPJ, C.CIDADE
FROM CLIENTE C
JOIN PJ ON C.CODIGO = PJ.CODIGO_CLI
ORDER BY C.NOME ASC, C.CIDADE DESC;

-- c. Listar os vendedores e seus pedidos já realizados com total de fatura entre R$ 1000,00 e R$ 5000,00, inclusive.
-- Mostrar: Nome do vendedor (VENDEDOR), número do pedido (PEDIDO), data do pedido (DATA) com a formatação 'dd/mm/yyyy'.
-- Ordenados pelo nome do vendedor descendente e, dentro deste, pela data ascendente.
-- RESOLUÇÃO:
